...
# 無槳測試用：稍微提高基準 thrust，避免太快 auto disarm
HOVER_THRUST = 0.50
THRUST_SCALE = 0.045
            ...
            try:
                ...
                if FIXED_CMD_TEST:
                    ...
                    if elapsed < SPOOL_TIME:
                        ...
                        target_roll = 0.0
                        target_pitch = 0.0
                        thrust_cmd = 0.22   # 原本 0.15，改高一點避免 auto disarm
                        is_triggered = True
                        ...
                        if not Z_ONLY:
                            ...
                            u_pitch, trig_x, _, _, _, _ = etm_pos_x.update(
                                np.array([err_b_x, err_b_vx], dtype=float), 0.0, dt
                            )
                            u_roll, trig_y, _, _, _, _ = etm_pos_y.update(
                                np.array([err_b_y, err_b_vy], dtype=float), 0.0, dt
                            )
