...
# 測試模式：
# 1) FIXED_CMD_TEST = True  -> 不跑控制器，直接送固定命令
# 2) Z_ONLY = True          -> 只開 Z 軸（XY 關）
# 3) XY_ONLY = True         -> 只開 XY 軸（thrust 固定）
# 4) 全部 False             -> 全控制
...
FIXED_TEST_THRUST = 0.4   # 原本 0.15，無槳時容易被飛控判定油門太低而 auto disarm
...
POS_ERR_MAX = 1.0
RATE_LIMIT_XY = 3.0
GAIN_SCALE_XY = 0.03       # 已經調降，實機測試時可視情況微調
...
class Altitude_ETM_Controller:
    def __init__(self):
        ...
        self.first_run = True
        self.last_trigger_time = 0.0  # 加入高度防線時間記錄
    ...
    def compute_control(self, current_state, target_height):
        ...
        # 加入 MIN_TRIGGER_INTERVAL，防止高度頻繁震盪觸發
        if self.first_run or (
            (term_net > SIGMA_ALT * term_trk) and
...
def generate_trajectory(home_x, home_y, elapsed):
    ...
    target_x = home_x + A * math.sin(w * elapsed)
    target_y = home_y + B * math.sin(w * elapsed) * math.cos(w * elapsed)
    target_vx = A * w * math.cos(w * elapsed)
    target_vy = B * w * math.cos(2.0 * w * elapsed)
    target_ax = -A * (w**2) * math.sin(w * elapsed)
    target_ay = -2.0 * B * (w**2) * math.sin(2.0 * w * elapsed)
                ...
                if FIXED_CMD_TEST:
                    ...
                    if elapsed < SPOOL_TIME:
                        ...
                        target_x, target_y, target_z, target_vx, target_vy, target_ax, target_ay = \
                            generate_trajectory(home_x, home_y, elapsed)
                        ...
                        if not XY_ONLY:
                            ...
                            u_accel, trig_z, _, _, _ = alt_ctrl.compute_control(current_alt_state, target_z)
                        ...
                        if not Z_ONLY:
                            ...
                            # 保留給 CSV Log 記錄用的 Body frame誤差 (不拿來算控制)
                            ...
                            # 【核心修正】：將地球座標 (Earth Frame) 的誤差直接餵給 ETM！
                            # 這樣 ETM 內部的參考模型 (ref_state) 才不會因為飛機旋轉而爆炸
                            u_x_earth, trig_x, _, _, _, _ = etm_pos_x.update(
                                np.array([err_w_x, err_w_vx], dtype=float), 0.0, dt
                            )
                            u_y_earth, trig_y, _, _, _, _ = etm_pos_y.update(
                                np.array([err_w_y, err_w_vy], dtype=float), 0.0, dt
                            )

                            # 旋轉控制力 (Earth -> Body) 解決馬桶效應
                            u_x_body = u_x_earth * cos_y + u_y_earth * sin_y
                            u_y_body = -u_x_earth * sin_y + u_y_earth * cos_y
                            ...
                            # 將 Body 推力轉為 Roll / Pitch 姿態角
                            u_pitch = u_x_body
                            u_roll = u_y_body

                            if z < XY_ENABLE_ALTITUDE:
                                target_roll = float(np.clip(
                                    -LOW_ALT_XY_GAIN * u_roll,
                                    -LOW_ALT_ROLL_PITCH_LIMIT,
                                    LOW_ALT_ROLL_PITCH_LIMIT
                                ))
                                target_pitch = float(np.clip(
                                    -LOW_ALT_XY_GAIN * u_pitch,
                                    -LOW_ALT_ROLL_PITCH_LIMIT,
                                    LOW_ALT_ROLL_PITCH_LIMIT
                                ))
                            else:
